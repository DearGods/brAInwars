...
const gameId = 3;
const waitForPlayersLimit = 5 * 1000;
let hash: Hash;
let gameLimit = 10 * 1000;

describe("5-game multiple players exit", () => {
  ...
  it("Third player joins", async () => {
    ...
    assert.equal(playersActions.players.length, 3);
  ...
  it("First player leaves", async () => {
    ...
    assert.equal(playersActions.players.length, 3);
    assert.equal(playersActions.playersActions.length, 4);
  });

  it("Second player leaves", async () => {
    const instruction = await leaveGameInstruction({
      connection: program.provider.connection,
      signer: crankSigner.publicKey,
      player: players[1].publicKey,
    ...
    assert.equal(playersActions.players.length, 3);
    assert.equal(playersActions.playersActions.length, 5);
  });
  ...
  it("Finish game", async () => {
    ...
    assert.equal(game.winner.toBase58(), players[1].publicKey.toBase58());
  ...
  it("Settle game", async () => {
    ...
    assert.equal(balancePost - balancePre, 3 * LAMPORTS_PER_SOL);
  });
});
